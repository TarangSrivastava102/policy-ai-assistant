...
REIMBURSEMENT_SHEET_NAME = (
    "Germane Media - Employee Reimbursements"
)

REIMBURSEMENT_DRIVE_FOLDER_NAME = (
    "Germane Media - Reimbursement Bills"
)
...
.block-container {
    ...
    padding: 28px 42px 40px !important;
}
...
.info-card {
    ...
    border-radius:16px;
    padding:18px 20px;
    box-shadow:0 5px 20px rgba(31,35,55,.035);
}
...
def google_clients():
    ...
    if not GOOGLE_LIBS_AVAILABLE:
        raise RuntimeError(
            ...
            "Add gspread, google-auth and google-api-python-client "
            "to requirements.txt."
        )

    # ========================================================
    # FIX:
    # Your secrets.toml currently contains:
    #
    # [google_service_account]
    #
    # Therefore we first look for GOOGLE_SERVICE_ACCOUNT_JSON
    # and then automatically fall back to the existing section.
    # ========================================================
    ...
    if not raw:
        raise RuntimeError(
            ...
            "Streamlit Secrets. Make sure your secrets contain "
            "[google_service_account]."
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Unable to read Google service-account credentials: {e}"
        )
    ...
    # Make sure escaped newlines are converted correctly if
    # the key happens to arrive as literal \\n characters.
    if "\\n" in info["private_key"]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Unable to create Google credentials: {e}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Unable to initialize Google Sheets/Drive: {e}"
        )
...
def get_or_create_spreadsheet():
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "Google Sheets spreadsheet could not be created. "
                f"Google error: {e}"
            )
...
def get_eligible_month_options(employee_email):
    ...
    last_idx = latest_claimed_month_index(
        employee_email
    )
...
def upload_to_drive(
    ...
    safe_employee = "".join(
        ...
        for c in employee_name
    ).strip()

    safe_type = "".join(
        c if c.isalnum() or c in " _-"
        else "_"
        for c in reimbursement_type
    ).strip()
    ...
    metadata = {
        ...
        "parents": [folder_id],
        "description": (
            f"Reimbursement bill submitted by "
            f"{employee_name} for {claim_month}."
        ),
    }
...
def smtp_is_configured():
    ...
    return (
        bool(get_secret("SMTP_EMAIL"))
        and bool(get_secret("SMTP_PASSWORD"))
    )
...
def send_reimbursement_hr_email(submission):
    ...
    if not smtp_is_configured():
        raise RuntimeError(
            "SMTP_EMAIL / SMTP_PASSWORD is not configured."
        )

    smtp_email = str(
        get_secret("SMTP_EMAIL")
    ).strip()
    ...
    for mkey in sorted(by_month.keys()):
        ...
        for item in items:
            ...
            <tr>
                <td style="padding:10px;border-bottom:1px solid #eee;">
                    {escape(item["reimbursement_type"])}
                </td>
        ...
        month_blocks.append(
            ...
            <div style="margin-top:8px;text-align:right;font-size:13px;color:#667085;">
                ...
                Month claimed:
                <b>₹{month_claimed:,.2f}</b>

                &nbsp;&nbsp; | &nbsp;&nbsp;

                Month eligible:
                <b style="color:#5c43d4;">
                    ₹{month_eligible:,.2f}
                </b>
    ...
    <div style="margin:0;padding:0;background:#f5f6fa;font-family:Arial,Helvetica,sans-serif;">
      ...
      <div style="max-width:760px;margin:0 auto;padding:28px 12px;">
        ...
        <div style="background:#173f70;border-radius:16px 16px 0 0;padding:24px 28px;color:#fff;">
          ...
          <div style="font-size:12px;letter-spacing:1px;text-transform:uppercase;opacity:.8;">
            Germane Media LLC
          </div>

          <div style="font-size:25px;font-weight:700;margin-top:6px;">
            New Reimbursement Submission
          </div>

          <div style="font-size:13px;opacity:.82;margin-top:5px;">
            Employee reimbursement request received
          </div>
        ...
        <div style="background:#fff;padding:26px 28px;border:1px solid #e6e8ee;border-top:0;border-radius:0 0 16px 16px;">
          ...
          <div style="display:flex;justify-content:space-between;gap:20px;flex-wrap:wrap;">
            ...
            <div>
              ...
              <div style="font-size:11px;color:#8a93a5;text-transform:uppercase;">
                Employee
              </div>

              <div style="font-size:16px;font-weight:700;color:#17233c;margin-top:3px;">
                {escape(submission["employee_name"])}
              </div>

              <div style="font-size:13px;color:#667085;margin-top:2px;">
                {escape(submission["employee_email"])}
              </div>
            ...
            <div style="text-align:right;">
              ...
              <div style="font-size:13px;font-weight:700;color:#17233c;margin-top:3px;">
                {escape(submission["submission_id"])}
              </div>

              <div style="font-size:13px;color:#667085;margin-top:2px;">
                {escape(submission["submission_date"])}
              </div>
          ...
          <div style="margin-top:26px;padding:18px;border-radius:13px;background:#f6f3ff;border:1px solid #e1d9ff;">
            ...
            <div style="font-size:11px;color:#6b5aa8;text-transform:uppercase;font-weight:700;">
                Total reimbursement requested
            </div>

            <div style="font-size:28px;font-weight:800;color:#5c43d4;margin-top:3px;">
                ₹{submission["total_eligible"]:,.2f}
            </div>

            <div style="font-size:12px;color:#667085;margin-top:3px;">
                Total invoice value:
                ₹{submission["total_claimed"]:,.2f}
            </div>
    ...
    msg["Subject"] = (
        f"Reimbursement Submission | "
        f"{submission['employee_name']} | "
        f"{submission['submission_id']}"
    )

    msg["From"] = formataddr(
        (
            "Germane Media Reimbursement Portal",
            smtp_email,
        )
    )
    ...
    msg.set_content(
        f"New reimbursement submission from "
        f"{submission['employee_name']}.\n"
...
def reimbursement_portal():
        ...
        st.markdown(
            ...
            <div class="info-card">
                ...
                <div class="info-value">
                    {escape(st.session_state.emp_name)}
                </div>
            </div>
            ''',
            unsafe_allow_html=True,
    ...
    with c2:
        ...
        st.markdown(
            ...
            <div class="info-card">
                ...
                <div class="info-value">
                    {escape(st.session_state.emp_email)}
                </div>
            </div>
            ''',
            unsafe_allow_html=True,
    ...
    with c3:
        ...
        submission_y, submission_m = (
            current_submission_month()
        )

        deadline = (
            f"{calendar.month_name[submission_m]} "
            f"{submission_y}"
        )
    ...
    st.success(
        f"Reimbursement submissions are open for "
        f"{calendar.month_name[submission_m]} "
        f"{submission_y} until "
        f"{calendar.month_name[submission_m]} "
        f"{REIMBURSEMENT_CUTOFF_DAY}."
    )
...
"""
    ...
    options = get_eligible_month_options(
    ...
    if not options:
        ...
        st.info(
            "No reimbursement months are currently available. "
            "Please check your previous reimbursement submission cycle."
        )
    ...
    st.markdown(
        ...
        <div class="section-head">
            <div class="section-title">
                1. Select reimbursement months
            </div>

            <div class="section-caption">
                Select up to 3 eligible months.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown(
        ...
        <div class="section-head">
            ...
            <div class="section-title">
                2. Add reimbursement details
            </div>

            <div class="section-caption">
                You can claim multiple reimbursement types for the same month.
            </div>
    ...
    for mkey in selected_keys:
        ...
        st.markdown(
            ...
            <div class="month-card">
                ...
                <div class="month-caption">
                    Choose one or more reimbursement types for this month.
                </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if not selected_types:
            ...
            st.caption(
                "No reimbursement type selected for this month."
            )
            ...
            for rtype in selected_types:
                ...
                st.markdown(
                    ...
                    <div class="type-card">
                        ...
                        <div class="type-subtitle">
                            {
                                '₹1,000 monthly cap • 1 invoice'
                                if config['monthly_limit']
                                else 'No monthly cap • multiple invoices allowed'
                            }
                        </div>
                    """,
                    unsafe_allow_html=True,
                ...
                if config["max_invoices"] == 1:
                    ...
                    if amount > 0 and eligible < amount:
                        ...
                        st.caption(
                            "Eligible amount is capped at "
                            "₹1,000 for this reimbursement "
                            "type and month."
                        )
                    ...
                    st.markdown(
                        ...
                        <div class="eligible-box">
                            ...
                            <div class="eligible-label">
                                Amount eligible for reimbursement
                            </div>

                            <div class="eligible-value">
                                ₹{eligible:,.2f}
                            </div>
    ...
    valid_items = [
        x
        for x in all_items
        if x["invoice_amount"] > 0
        or x["file"] is not None
    ]
    ...
    st.markdown(
        ...
        <div class="section-head">
            ...
            <div class="section-caption">
                The final summary will be sent to HR after submission.
            </div>
            ...
            <div class="review-row">
                ...
                <div style="display:flex;justify-content:space-between;gap:20px;">
                    ...
                    <div class="review-amount">
                        ...
                        <div style="font-size:11px;color:#5c43d4;">
                            Eligible ₹{item["eligible_amount"]:,.2f}
                        </div>
    ...
    st.markdown(
        ...
        <div class="total-card">
            ...
            <div class="total-label" style="margin-top:15px;">
                Total reimbursement requested
            </div>
            ...
            <div class="total-note">
                Subject to HR verification and approval.
            </div>
    ...
    if submit:
        ...
        if not selected_keys:
            ...
            errors.append(
                "Please select at least one reimbursement month."
            )

        if len(selected_keys) > 3:

            errors.append(
                "You can claim a maximum of 3 months."
            )

        for mkey in selected_keys:

            types = st.session_state.get(
                f"types_{mkey}",
                [],
            )
            ...
            if not types:
                ...
                errors.append(
                    f"Please select at least one reimbursement type "
                    f"for {month_label(y,m)}."
                )
        ...
        for item in all_items:
            ...
            if item["invoice_amount"] <= 0:
                ...
                errors.append(
                    f"Please enter a valid amount for "
                    f"{label} → "
            ...
            if item["file"] is None:
                ...
                errors.append(
                    f"Please upload the supporting document for "
                    f"{label} → "
                    f"{item['reimbursement_type']} → "
                    f"Invoice {item['invoice_no']}."
                )
        ...
        existing = get_employee_submissions(
            st.session_state.emp_email
        )
        ...
        if duplicate_months:
            ...
            errors.append(
                "The following month(s) have already "
                "been submitted: "
                + ", ".join(
                    month_label(
                        *parse_month_key(x)
                    )
                    for x in duplicate_months
                )
            )
        ...
        with st.spinner(
            "Uploading bills and saving your reimbursement..."
        ):
            ...
            try:
                ...
                send_reimbursement_hr_email(
                    submission
                )

                st.session_state.reimbursement_success = (
                    submission
                )

                st.session_state.reimbursement_sheet_url = (
                    sheet_url
                )
    ...
    if st.session_state.get(
        ...
        st.markdown(
            ...
            <div class="success-card">
                ...
                <div class="success-text">
                    ...
                    <b>{escape(submission["submission_id"])}</b>
                    has been recorded.

                    HR has been notified by email and the
                    supporting documents have been stored
                    in Google Drive.
                </div>
        ...
        for mkey in sorted(
            ...
            st.write(
                f"**{month_label(y,m)}** — "
                f"Claimed ₹{month_claimed:,.2f} • "
                f"Eligible ₹{month_eligible:,.2f}"
            )

        st.success(
            f"Total reimbursement requested: "
            f"₹{submission['total_eligible']:,.2f}"
        )
...
if not st.user.is_logged_in:
    ...
    if logo_path.exists():
        ...
        logo_html = (
            ...
            'border-radius:20px;background:#6547ed;'
            'color:#fff;display:flex;align-items:center;'
            'justify-content:center;font-size:50px;'
            'font-weight:800;">G</div>'
        )
    ...
    st.markdown(
        ...
        <div style="max-width:1050px;margin:80px auto;display:grid;grid-template-columns:1.1fr .9fr;gap:45px;align-items:center;">
            ...
            <div>
                ...
                <div style="display:flex;align-items:center;gap:18px;">
                    ...
                    <div>
                        ...
                        <div style="font-size:30px;font-weight:800;color:#17233c;">
                            Germane Media LLC
                        </div>

                        <div style="color:#6547ed;font-weight:700;margin-top:5px;">
                            Internal HR Portal
                        </div>
                ...
                <h1 style="font-size:32px;color:#17233c;margin-top:48px;">
            ...
            <div style="background:#fff;border:1px solid #e3e6ee;border-radius:22px;padding:38px;box-shadow:0 14px 40px rgba(32,35,58,.08);">
                ...
                <div style="text-align:center;font-size:30px;font-weight:800;color:#17233c;margin-top:12px;">
                    Welcome Back!
                </div>

                <div style="text-align:center;color:#747d8e;margin:8px 0 25px;">
...
if not Path(POLICY_PDF).exists():
    ...
    st.info(
        "Upload GERMANE_MEDIA_LLC_POLICY_DOCUMENT.pdf "
...
with col_main:
    ...
    st.markdown(
        '<div class="brand-sub">'
        'Ask questions, verify rules, and connect with HR.'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        <div class="info-card">
            ...
            <span style="font-size:12px;color:#667085;">
                ...
                Your chat session is associated with
                your authenticated employee account.

                HR may access transcripts for support
            ...
            if message["role"] == "assistant":
                ...
                if (
                    ...
                    st.warning(
                        ...
                        "resolve your question. You can "
                        "contact HR or schedule a "
                    ...
                    with e1:
                        ...
                        if st.button(
                            ...
                            try:
                                ...
                                st.success(
                                    "Your conversation has been sent to HR."
                                )

                            except Exception as e:

                                st.error(
                                    f"Unable to notify HR: {e}"
                                )
            ...
            st.session_state.messages.append(
                {
                    ...
                    "content": (
                        response
                        + "\n\n---\n"
                        "*Notice: Answers are derived from the "
                        "Germane Media LLC Policy Handbook. "
